/* Lamiah Khan DSA II, 09/15/2024
 ...
 Content from hash.h file and makefile were kept the same. Hash function from DSA Analysis in C++ (Fourth Edition) 
 Any other resources used will be linked to within the document.*/

/*NOTE: i built my code on vscode, but i compiled it using cygwin on my linux machine. Git Bash did not produce proper results (errors on makefile)*/
...
bool hashTable::rehash(){
    ...
    for(int j = 0; j < prevData.size(); j++){
        if(prevData[j].isOccupied){
            insert(prevData[j].key);}}
            prevData.clear();}
            return true;}
